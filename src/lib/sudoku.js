...
export function checkInstance(sudoku,i,j,val){
  const candidate = sudoku.rows
  .map(row => row.cols.map(col => col.value))
  .flat();
  
  if(candidate[i*9+j]>0){
  if(candidate[i*9+j]!==sudoku.solution[i*9+j])
    {
     <input className="err" value={candidate[i*9+j]}></input>
    sudoku.errors=sudoku.errors+1;
    console.log('errors:',sudoku.errors)
    return true
  }
  else {
  return false
